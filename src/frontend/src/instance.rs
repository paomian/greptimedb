...
impl FrontendInstance for Instance {
    async fn start(&mut self) -> Result<()> {
        ...
        let mut conf = None;
        if let Some(configurator) = plug.get::<ConfiguratorRef>().cloned() {
            let g = configurator.lock().await;
            conf = g.clone();
        }
        futures::future::try_join_all(self.servers.values().map(|x| start_server(x, conf.clone())))
            .await
